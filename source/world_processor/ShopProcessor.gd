...
onready var shop_scene = preload("res://source/dummy_shop/DummyShop.tscn")
...
func add_to_the_tree():
	for key in collection.keys():
		if not container.has_node(str(key)):
			var new_loot = shop_scene.instance()
			new_loot.name = str(key)
			container.call_deferred("add_child", new_loot, true)
			new_loot.update(collection[key]["pos"], collection[key]["goods"])
...
func process_data():
	var wsb2_loot = get_parent().world_state_buffer[2]["S"]
